...
# Fungsi untuk menghitung nilai fungsi dari ekspresi matematika yang diberikan
def hitung_fungsi(fungsi, x):
    x_symbol = sp.symbols('x')
    fungsi_expr = sp.sympify(fungsi)
    return fungsi_expr.subs(x_symbol, x)
...
def bagiDua(fungsi, x0, x1, galat, iterasi_maksimal):
    i = 1
...
# Input dari pengguna
# Program utama dimulai dengan input dari pengguna, termasuk fungsi, tebakan awal (x0 dan x1), galat yang diinginkan, dan jumlah iterasi maksimal.
fungsi = input('Masukkan fungsi (contoh: x**3 + 4*x**2 - 10): ')
...
if f(x0) * f(x1) > 0.0:
    ...
    x_values = bagiDua(fungsi, x0, x1, galat, iterasi_maksimal)
